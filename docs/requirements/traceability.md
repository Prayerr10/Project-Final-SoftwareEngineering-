...
| NFR-02 | Semua user story | ARCH-02, ARCH-12 | API-01 sampai API-17 | UI API-to-UI mapping | #13, #20, #21, #23 | `worker/index.ts`, `src/App.tsx` | `tests/integration/worker-health.test.ts`, `tests/integration/dashboard-summary.test.ts`, `tests/integration/react-foundation.test.ts`, `tests/integration/role-validation-states.test.ts` | Selesai; Issue #21 verifies Worker role/action validation responses and React role mapping |
| NFR-03 | Semua user story | ARCH-03, ARCH-12 | DB-01 sampai DB-07 | UI data display from DB/API contracts | #13, #20, #23 | `worker/index.ts`, `database/migrations/0001_initial.sql`, `wrangler.jsonc` | `tests/integration/worker-health.test.ts`, `tests/integration/dashboard-summary.test.ts` | Selesai; Issue #20 dashboard reads summary and workload source data from D1-backed tables |
...
| NFR-06 | Semua user story | ARCH-04 | Contract-first validation and error contracts | UI-08, UI-09 state/error design | #22 | `worker/index.ts`, `src/App.tsx` | `tests/integration/role-validation-states.test.ts` | Selesai untuk Issue #21 slice; forbidden, validation, and invalid-transition states are covered |
| NFR-07 | Semua user story | ARCH-13 | Skill 07 traceability links added | Skill 08 traceability links added | #24 | `docs/requirements/traceability.md` | `tests/integration/role-validation-states.test.ts` | Selesai untuk Issue #21 slice; role-based UI/API validation traceability updated |
